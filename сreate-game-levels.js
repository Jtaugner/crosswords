...
const allWords = require('./simpleWords.js').words;
...
const stupidWords = ['фильм', 'повесть', 'рассказ', 'сборник', 'книга', 'роман', 'картина', 'пьеса', 'новелла', 'оперетта', 'стихотворение', 'поэма'];
...
function getLevelFromWords(arr) {
    ...
    wordsArray.forEach((word, index)=>{
        console.log(word);
        let desc = wordsDescriptions[word][Math.floor(Math.random()*wordsDescriptions[word].length)];
        while(isWordHaveStupidWords(desc) && Math.random() > 0.2){
            desc = wordsDescriptions[word][Math.floor(Math.random()*wordsDescriptions[word].length)];
        }
...
const randAmount= [9,10,11,12,13];
let levelsAll = '';
for (let i = 0; i < 140; i++){
    ...
    levelsAll += `${JSON.stringify(getLevel(length, amount))} ,\n`;
}
// const all6Words = [];
// shuffle(allWords);
// for(let i = 0; i < allWords.length; i++){
